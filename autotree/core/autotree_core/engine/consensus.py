"""Deterministic content agreement scores for live generation branches."""
...
def consensus_scores(
    ...
    """Score live branches by answer agreement or trailing lexical overlap."""
    ...
    answers = {
        branch_id: extract_final_answer(partial_texts[branch_id])
        for branch_id in branch_ids
    }
    if any(answer is not None for answer in answers.values()):
        raw_scores = _answer_group_scores(branch_ids, answers)
    else:
        raw_scores = _lexical_scores(branch_ids, partial_texts, config.trailing_window)

    return _protect_minimum_survivors(raw_scores, config.min_survivors)
...
def _protect_minimum_survivors(
    ...
    ranked = sorted(raw_scores, key=lambda branch_id: (-raw_scores[branch_id], branch_id))
    protected = set(ranked[: min(min_survivors, len(ranked))])
    return {
        branch_id: best_score if branch_id in protected else raw_scores[branch_id]
        for branch_id in sorted(raw_scores)
    }
